...
docs/depiction.json         native package details; release notes generated by Pages
...
## Package Details and Pages

Pages deploys `docs/` through GitHub Actions, not the legacy branch source.
`packaging/DEBIAN/control` advertises the web and native depiction URLs.
Keep `docs/depiction.json` accurate for this tool and its supported iOS versions.
Use the largest actual README banner as `headerImage` when one exists; omit
that field when there is no artwork. The pinned shared updater in
`owngoal-dev/owngoal-packages` fetches release notes at deploy time. Never
hand-maintain the generated Changelog tab. Keep the Pages workflow’s
`workflow_run` name aligned with `Release`, including after renaming it.

CLI packages need no app-registration hooks. For a package that includes an
app bundle, retain a `uikittools` dependency and use its registration triggers;
do not call `uicache` from installation or removal scripts.
